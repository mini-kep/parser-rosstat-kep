...
def stream_table_rows_images(df):
    df = df.drop(['year', 'month'], 1)
    for vn in df.columns:
        ts = df[vn]
        sparkline = Sparkline(ts)
        sparkline.save()
        yield [sparkline.markdown()]


#  # создаем таблицу из 1 колонки (заголовок: "", содержимое: пути до

# изображений, созданных выше в write_sparkline_pngs)
md3 = to_markdown(body=stream_table_rows_images(dfm), header=[""])

# writing images_paths.md
monthy_images.write_text(md3)
print(md3)
