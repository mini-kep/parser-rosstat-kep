...
def validate_marker(marker):
    ...
    return start_found and end_found and (
        (start_head != end_head) or (
            start_pos < end_pos))
...
def test_cfg_additional_markers_valid():
    ...
    for definition in additional_definitions:
        for marker in definition.markers:
            if marker["start"] is None or marker["end"] is None:
                none_markers_notices.append(
                    "definition: '{}'; marker: '{}';".format(
                        definition, marker))
    ...
    for definition in additional_definitions:
        ...
        if not validate_marker(marker):
            invalid_markers_notices.append(
                "definition '{}'; marker not found: '{}'".format(
                    definition, marker))
